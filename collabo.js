...
function signup() {
  ...
  if (passwo != confirm) {
    intext.innerText =
      "The passwords inputed are different,Kindly reconfirm the password";
  } else if (
    fnam.value == "" ||
    snam.value == "" ||
    emai.value == "" ||
...
const notelist = () => {
  ...
  currentClientNote.map((item, i) => {
    tab.innerHTML += `<div class='margin'> (Page ${
      i + 1
    }) <button class='del' onclick="deleteNote(${i})"><i class="fa fa-trash" aria-hidden="true"></i></button><br>
